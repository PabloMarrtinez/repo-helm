...
const caCertPath = '/usr/share/ca-certificates/';
...
async function getAccessToken(req_session, res, authCode) {
	...
	// Read all .crt files from the directory
    const caCerts = fs.readdirSync(caCertPath)
                      .filter(file => file.endsWith('.crt'))
                      .map(file => fs.readFileSync(path.join(caCertPath, file)));
	...
	var req = {
		...
		agentOptions: {
            ca: caCerts  // Trust the CA certificates from the directory
        }
	}
